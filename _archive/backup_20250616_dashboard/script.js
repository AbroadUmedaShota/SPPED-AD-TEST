...
document.addEventListener('DOMContentLoaded', () => {
    ...
    function initializeFloatingLabelsForModal(containerElement) { // 汎用化
        containerElement.querySelectorAll('.input-group .input-field').forEach(inputField => {
            // --- 変更点: ロード時とオートフィル時のラベル調整を強化 ---
            // 値がセットされているか、またはオートフィルされた場合にnot-placeholder-shownを即座に付与
            if (inputField.value !== '' || inputField.matches(':-webkit-autofill')) {
                inputField.classList.add('not-placeholder-shown');
            } else {
                inputField.classList.remove('not-placeholder-shown');
            }
            // --- End 変更点 ---
        ...
        containerElement.querySelectorAll('.input-group .input-field-static').forEach(inputField => {
            inputField.classList.add('not-placeholder-shown'); // Keep label active for static fields
        });
    }
        ...
        surveysToRender.forEach(survey => {
            ...
            row.innerHTML = `
                <td data-label="アクション" class="px-4 py-3 whitespace-nowrap actions-cell">
                    ...
                    <button class="text-secondary hover:bg-secondary-container rounded-full p-2 transition-all shadow-sm shadow-soft" title="アンケートをコピー" aria-label="アンケートをコピー"><span class="material-icons text-base">content_copy</span></button>
            ...
            row.querySelector('button[title="アンケートを編集"]').addEventListener('click', (e) => {
                e.stopPropagation();
                // Pass survey data to openSurveyDetails
                // --- 変更点: 相対パスに変更 ---
                window.handleOpenModal('surveyDetailsModal', 'modals/surveyDetailsModal.html')
                      .then(() => window.populateSurveyDetails(survey)) // Populate details after modal is loaded and opened
                      .catch(error => console.error("Failed to open or populate survey details modal:", error));
            });
            ...
            row.querySelector('button[title="アンケートをコピー"]').addEventListener('click', (e) => {
                e.stopPropagation();
                showToast(`アンケートID: ${survey.id} をコピーします。（実装はここから）`, 'info');
            });
    ...
    function populateAccountInfoModal(userData) {
        ...
        // --- 変更点: データ投入後にinputイベントを強制的に発火させる ---
        // これにより、ブラウザのオートフィルやJSによる値設定後もFloating Labelが正しく移動する
        modal.querySelectorAll('.input-group .input-field').forEach(inputField => {
            // inputイベントをディスパッチ (bubbles: true でイベントがバブリングするようにする)
    ...
    window.populateSurveyDetails = function(survey) {
        ...
        const detail_surveyId = document.getElementById('detail_surveyId');
        const detail_surveyNameInternal = document.getElementById('detail_surveyNameInternal');
        const detail_displayTitle = document.getElementById('detail_displayTitle');
        const detail_plan = document.getElementById('detail_plan');
        const detail_surveyMemo = document.getElementById('detail_surveyMemo');
        ...
        const detail_dataCompletionDate = document.getElementById('detail_dataCompletionDate');
        const detail_deadline = document.getElementById('detail_deadline');
        const detail_estimatedBillingAmount = document.getElementById('detail_estimatedBillingAmount');

        const detail_bizcardEnabled = document.getElementById('detail_bizcardEnabled');
        const detail_bizcardCompletionCount = document.getElementById('detail_bizcardCompletionCount');
        const detail_thankYouEmailSettings = document.getElementById('detail_thankYouEmailSettings');
        ...
        if (detail_surveyId) detail_surveyId.textContent = survey.id;
        if (detail_surveyNameInternal) detail_surveyNameInternal.textContent = survey.name;
        if (detail_displayTitle) detail_displayTitle.textContent = survey.displayTitle || 'なし';
        if (detail_plan) detail_plan.textContent = survey.plan || 'N/A';
        if (detail_surveyMemo) detail_surveyMemo.textContent = survey.memo || survey.description || 'なし';
        ...
        if (detail_answerCount) {
            const realtimeAnswersDisplay = survey.realtimeAnswers > 0 && (survey.status.includes('会期中') || survey.status.includes('データ化中'))
                                           ? ` (+${survey.realtimeAnswers})`
                                           : '';
            detail_answerCount.textContent = `${survey.answerCount}${realtimeAnswersDisplay}`;
        }
        
        if (detail_dataCompletionDate) detail_dataCompletionDate.textContent = survey.dataCompletionDate || '未定';
        if (detail_deadline) detail_deadline.textContent = survey.deadline || 'N/A';
        if (detail_estimatedBillingAmount) detail_estimatedBillingAmount.textContent = survey.estimatedBillingAmount ? `¥${survey.estimatedBillingAmount.toLocaleString()}` : 'N/A';


        // Populate business card info
        if (detail_bizcardEnabled) detail_bizcardEnabled.textContent = survey.bizcardEnabled ? '利用する' : '利用しない';
        if (detail_bizcardCompletionCount) {
            if (survey.bizcardEnabled && survey.bizcardCompletionCount !== undefined) {
                detail_bizcardCompletionCount.textContent = `${survey.bizcardCompletionCount}件`;
            } else {
                detail_bizcardCompletionCount.textContent = 'N/A';
            }
        }
        if (detail_thankYouEmailSettings) detail_thankYouEmailSettings.textContent = survey.thankYouEmailSettings ? survey.thankYouEmailSettings : '設定なし';

        // Populate access info
        const qrUrl = `https://survey.speedad.com/qr/${survey.id}`;
        const qrImageUrl = `https://via.placeholder.com/200x200?text=QR+Code+${survey.id}`;
        ...
        if (downloadQrCodeBtn) {
            // Remove previous listener to prevent duplicates and re-attach with current survey context
            const currentDownloadQrCodeBtn = downloadQrCodeBtn.cloneNode(true);
            downloadQrCodeBtn.parentNode.replaceChild(currentDownloadQrCodeBtn, downloadQrCodeBtn);
            currentDownloadQrCodeBtn.addEventListener('click', () => {
        ...
        if (detailDownloadBtn) {
            // Remove previous listener to prevent duplicates and re-attach with current survey context
            const currentDetailDownloadBtn = detailDownloadBtn.cloneNode(true);
            detailDownloadBtn.parentNode.replaceChild(currentDetailDownloadBtn, detailDownloadBtn);
            currentDetailDownloadBtn.addEventListener('click', () => {
        ...
        if (editButton) {
            const currentEditButton = editButton.cloneNode(true);
            editButton.parentNode.replaceChild(currentEditButton, editButton);
        ...
        if (analyzeButton) {
            const currentAnalyzeButton = analyzeButton.cloneNode(true);
            analyzeButton.parentNode.replaceChild(currentAnalyzeButton, analyzeButton);
            currentAnalyzeButton.addEventListener('click', () => {
    ...
    // Table row click event delegation (remains unchanged mostly, but calls handleOpenModal internally)
    document.querySelectorAll('#surveyTableBody').forEach(tableBody => {
        tableBody.addEventListener('click', (event) => {
            ...
            if (surveyId) {
                // Call handleOpenModal for surveyDetailsModal
                // --- 変更点: 相対パスに変更 ---
                window.handleOpenModal('surveyDetailsModal', 'modals/surveyDetailsModal.html')
                      .then(() => {
                          // Populate details AFTER the modal is confirmed to be loaded and opened
                          const survey = allSurveyData.find(s => s.id === surveyId);
                          if (survey) {
                              window.populateSurveyDetails(survey);
                          } else {
                              console.error(`Survey with ID ${surveyId} not found in allSurveyData.`);
                              showToast(`アンケートID: ${surveyId} のデータが見つかりませんでした。`, "error");
                          }
                      })
                      .catch(error => console.error("Failed to open or populate survey details modal due to loading error:", error));
            }
        });
    });
