...
| `plan` | String | ⬜️ | 契約されているプラン名。機能制限などに関わる可能性があります。 | `"スタンダードプラン"` |
...
| `bizcardCompletionCount`| Number | ⬜️ | データ化が完了した名刺の枚数です。`bizcardEnabled`が`true`の場合に意味を持ちます。 | `120` |
...
| `permissions` | Object | ✅ | このグループに適用される権限設定。下記 `Permissions` オブジェクトです。 |
...
### 4.2. 権限オブジェクト (`Permissions`)

`Group` オブジェクトの `permissions` プロパティに含まれる、権限の詳細設定を表すオブジェクトです。

| プロパティ名 | データ型 | 必須 | 説明 |
| :--- | :--- | :--- | :--- |
| `allowBillingInfoView` | Boolean | ✅ | グループのメンバーが請求情報の閲覧を許可されるかどうか。 |
