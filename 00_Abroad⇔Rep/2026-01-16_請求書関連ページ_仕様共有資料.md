...
- プレミアムプランの月額費用は「プレミアムプランを使用しているアカウント」に対して請求される（アカウント依存）。
- グループ全体の請求（`AGG-YYYYMM-GROUP`）には、プレミアムプランの費用は合算されない。
- プレミアムプランは月額固定であり、プランに含まれる高度なオプション機能に対して従量課金や追加単価は発生しない。
