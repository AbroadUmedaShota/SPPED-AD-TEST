...
function main() {
  ...
  const surveyFile = path.join(base, 'data/demo/demo_surveys', `${SURVEY_ID}.json`);
  const answerFile = path.join(base, 'data/demo/demo_answers', `${SURVEY_ID}.json`);
  const bizFile = path.join(base, 'data/demo/demo_business-cards', `${SURVEY_ID}.json`);
