    ...
    <aside class="h-fit rounded-lg border border-outline-variant bg-surface p-4 lg:sticky lg:top-4">
      <div class="space-y-3">
        <label class="flex flex-col gap-1 text-sm text-on-surface-variant">
          <span>検索</span>
          <input id="docViewerSearch" type="search" class="form-input" placeholder="タイトル・カテゴリ・本文で検索">
        </label>
        <div class="grid grid-cols-1 gap-3 sm:grid-cols-2 lg:grid-cols-1">
          <label class="flex flex-col gap-1 text-sm text-on-surface-variant">
            <span>カテゴリ</span>
            <select id="docViewerCategory" class="form-select">
              <option value="">すべて</option>
            </select>
          </label>
          <label class="flex flex-col gap-1 text-sm text-on-surface-variant">
            <span>ソース状態</span>
            <select id="docViewerSourceStatus" class="form-select">
              <option value="">すべて</option>
            </select>
          </label>
        </div>
        <label class="flex items-center gap-2 text-sm text-on-surface">
          <input id="docViewerDiffOnly" type="checkbox" class="form-checkbox rounded border-outline">
          差分・未確認のみ
        </label>
      </div>
      <div class="mt-4 flex items-center justify-between text-xs text-on-surface-variant">
        <span id="docViewerCount">0件</span>
        <button id="docViewerReset" type="button" class="inline-flex items-center gap-1 text-primary hover:underline">
          <span class="material-icons text-sm">restart_alt</span>
          条件リセット
        </button>
      </div>
      <div id="docViewerList" class="mt-3 max-h-[60vh] space-y-2 overflow-y-auto pr-1" aria-label="資料一覧"></div>
    </aside>
