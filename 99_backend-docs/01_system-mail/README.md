...
- `customer@speed-ad.com` は Google Workspace 上の問い合わせ用 mailbox として扱う。サポート問い合わせフォームの受信・返信・通知は `docs/画面設計/仕様/23_support_contact_form_requirements.md` を正とする。
- `info@speed-ad.com` は既存サービスの自動配信用送信元として扱う。Xserver SMTP を継続利用する場合も、問い合わせフォーム用の `customer@speed-ad.com` とは用途を分ける。
- `speed-ad.com` の受信 MX は Google Workspace 側へ寄せ、Xserver は受信基盤ではなく既存送信用途として残す方針とする。
...
| ID | 資料 | 状態 |
| --- | --- | --- |
| SM-001 | [新規アカウント作成時（配信なし）](./01_account-registration.md) | 要件変更により配信なし |
