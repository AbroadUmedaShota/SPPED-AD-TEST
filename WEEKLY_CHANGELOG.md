# 変更履歴 (2025-07-22 〜 2026-02-05)

## 2026-02-05 (木)

- **プレミアムプラン定義の具体化と制限マトリクスの刷新**:
    - **月額10,000円プランの確立**: プレミアムプランを「月額10,000円（税別）」の単一プランとして再定義し、先行モニター企業向け展開（5/18）を見据えた構成に整理しました。
    - **ダウンロード制限の明確化**:
        - 無料（Free）プラン：回答テキストのみダウンロード可能。
        - プレミアム（Premium）プラン：名刺画像、設問添付画像（第3の画像）、名刺・回答結合データ、および分析Excelレポートの出力が可能。
    - **Abroad・Rep間の役割分担の明文化**:
        - Abroadチーム：企画、プロダクト設計、および静的なページ（加入LP等）を担当。
        - Repチーム：動的なロジック、API、およびシステム開発全般を担当。
- **ドキュメント間の整合性確保**:
    - **プラン別機能制限仕様書の更新**: `docs/product/specs/11_plan_feature_restrictions.md` を最新のプラン定義と役割分担、制限マトリクスに合わせて全面的に刷新しました。
    - **機能一覧および粒度分解資料の更新**: `docs/service-consideration/` 内の各資料について、決定したダウンロード仕様と作業区分を反映しました。
- **対象ファイルの更新**:
...
- **詳細分析画面の機能強化と描画エンジン刷新**:
    - **ApexChartsへの完全移行**: 描画ライブラリをChart.jsからApexCharts（SVGベース）に変更し、より滑らかなアニメーションとインタラクティブな操作感、および高解像度な出力に対応しました。
    - **グラフ種類の動的切り替え**: ユーザーが用途に合わせて棒グラフと円グラフを瞬時に切り替えられる機能を実装しました。
    - **凡例のインテリジェント配置**: ラベルの長さや項目数に基づき、凡例を右側または下部に自動で最適配置するロジックを導入しました。
    - **Excelレポート出力機能の実装**: `ExcelJS`を使用して、全設問の集計データとグラフ画像を個別のシートにまとめた詳細レポートを一括出力する機能を実装しました。
    - **フローティング・プログレス・バー**: 重い出力処理中も操作を妨げない、ノンブロッキングな進行状況表示UIを導入しました。
- **SPEEDレビュー画面の視認性向上**:
    - **データ化ステータスの強調表示**: データ化進行中のレコードに対し、専用の背景色とアニメーションドットを適用し、進捗状況を一目で把握できるようにしました。
- **システム全体の視覚的統一感の向上**:
    - **ページタイトルの標準化**:
        - プロジェクト内の全ページの `<title>` タグを `SPEED-AD - [ページ名]` の形式に統一しました。これにより、ブラウザのタブや履歴における一貫性を確保しました。
- **アンケート一覧画面・詳細モーダルのUI改善**:
    - **複製アクションの配置変更**:
        - アンケート一覧テーブルのアクション列から「複製」ボタンを削除しました。
        - アンケート詳細モーダルの下部「データダウンロード」ボタンの右側に「複製」ボタンを新設しました。これにより、詳細情報を確認した後にワンクリックで複製を行えるよう導線を整理しました。
- **プロジェクト憲法およびドキュメント管理の刷新**:
    - **ドキュメント構造の整合性確保**: プロジェクトの実態（`docs/product/` 等への整理）に合わせて、`GEMINI.md` 内の全てのドキュメント参照パスを最新の状態にアップデートしました。
    - **プロジェクト・ポータルの新設**: ルートディレクトリに `README.md` を作成し、プロジェクトの全体像、ディレクトリ構造、および各主要ドキュメントへのクイックアクセスを提供しました。これにより、AIおよび開発者のオンボーディング効率を向上させました。
...
- **詳細分析画面 (`graph-page.html`) のUI/UX最終調整**:
    - **スマート・フィルター・バーの導入**: 煩雑なタブ形式を廃止し、「表示期間」ドロップダウンを中心とした1列のツールバー形式に刷新しました。カスタム範囲指定時のみ詳細入力が展開されるスマートな挙動を実現しました。
    - **バーティカル・インサイト・レイアウトの確立**: 表とグラフの横並びを廃止し、カード内で縦に積み上げる構成に変更しました。これにより、特に2列表示時の可視性と情報の読み取りやすさが劇的に向上しました。
    - **表示設定のシームレスな統合**: フィルターエリア内にチェックボックス形式で表示設定を統合し、分析の流れを止めずに見た目のカスタマイズが可能になりました。
    - **フローティングラベルの完全実装**: 入力欄のラベルが動的に浮かび上がるアニメーションを導入し、Material 3準拠のモダンな体験を提供します。
    - **レイアウトのガタつき解消**: プレースホルダーの高さ適正化により、ページ読み込み時のCLS（Cumulative Layout Shift）を排除しました。
