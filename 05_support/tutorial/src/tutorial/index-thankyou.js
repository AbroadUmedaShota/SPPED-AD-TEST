...
// - グローバル API window.SpeedAD.tutorial 設置（guards-thankyou.js 経由）
// - after_event_ready 状態固定は guards-thankyou.js が DOMContentLoaded 後に処理
// - #saveThankYouEmailSettingsBtn を capture フェーズでインターセプトして本番保存を抑止
// - #sendThankYouEmailBtn を capture フェーズでインターセプト（二重防御）

import { TUTORIAL_STEPS, TOTAL_STEPS, getStepById, TUTORIAL_CONFIG } from './steps-thankyou.js';
import {
  initOverlay,
  destroyOverlay,
  setCallbacks,
...
document.addEventListener('DOMContentLoaded', async () => {
  ...
  // #saveThankYouEmailSettingsBtn を capture フェーズでインターセプト
  document.addEventListener('click', (ev) => {
    if (!api.isActive()) return;
    const saveBtn = ev.target.closest('#saveThankYouEmailSettingsBtn');
    if (saveBtn) {
