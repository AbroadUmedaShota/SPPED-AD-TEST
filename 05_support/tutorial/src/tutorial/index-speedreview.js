// index-speedreview.js
// SPEEDレビュー チュートリアルの起動制御。
//
// - ?tutorial=1 検出 / 共通 HTML 注入の await / 各モジュール初期化
// - グローバル API window.SpeedAD.tutorial 設置（guards-speedreview.js 経由）
// - 回答詳細モーダルの「保存する」(#saveDetailBtn) を capture フェーズでインターセプトし、
//   練習中は実際の更新を行わない（デモデータを汚さない）
// - ハブから直接起動するため、初回はようこそ画面を挟む。完了・スキップはハブへ戻る。

import { TUTORIAL_STEPS, TOTAL_STEPS, getStepById, TUTORIAL_CONFIG } from './steps-speedreview.js';
import {
  initOverlay,
  destroyOverlay,
  setCallbacks,
...
import { installGlobalApi, enableTargetForTutorial } from './guards-speedreview.js';
...
document.addEventListener('DOMContentLoaded', async () => {
  ...
  // 回答詳細モーダルの「保存する」を capture フェーズでインターセプトして本番更新を抑止。
  // 保存ボタンは編集モード時に updateModalFooter が動的生成する（id=saveDetailBtn）。
  document.addEventListener('click', (ev) => {
    if (!api.isActive()) return;
    const btn = ev.target.closest && ev.target.closest('#saveDetailBtn');
    if (!btn) return;
    ev.stopImmediatePropagation();
    ev.preventDefault();
    try { showToast('練習モードのため、変更は保存されません。', 'info'); } catch (_e) { /* noop */ }
  }, true /* capture */);
  ...
  // 初回起動（進行状態なし & 開始ステップ=1）はようこそ画面を挟む。
  // 中断再開・URL での step 指定時は直接該当ステップへ。
  const progress = readProgress(TUTORIAL_CONFIG.progressKey);
  const isFreshStart = !progress && startStep === 1;
  if (isFreshStart) {
    showWelcome({
      eyebrow: 'SPEEDレビュー チュートリアル',
      title: 'ようこそ',
      lead: 'SPEEDレビューは、集めた回答と名刺をその場で確認・集計・編集できる画面です。',
      invite: 'これから画面の見方を、数分で体験しましょう。',
      skipLabel: 'スキップ',
      onStart: () => goToStep(1),
...
function goToStep(stepId) {
  ...
  if (step.onEnterAction) {
    applyEnterAction(step.onEnterAction);
  }

  resolveTargetAndRender(step);
}

// ステップ進入時アクション。フィルタ系ステップでは右サイドバーを確実に開く。
function applyEnterAction(action) {
  if (action === 'openRightSidebar') {
    ensureRightSidebarOpen();
  }
}

// 右サイドバー（フィルターパネル）は初期表示後 2.5 秒で自動的に閉じる仕様のため、
// チュートリアルのフィルタ説明ステップでは対象が画面外へ格納されてしまう。
// 自動クローズを無効化し、閉じている場合のみ開く（トグルは状態反転なので、
// 開いているときに押すと逆に閉じてしまう。translate-x-full＝閉状態をクラスで判定して防ぐ）。
function ensureRightSidebarOpen() {
  const sidebar = document.getElementById('right-sidebar');
  const toggle = document.getElementById('right-sidebar-toggle-btn');
  if (!sidebar || !toggle) return;
  if (sidebar._autoCloseTimer) {
    window.clearTimeout(sidebar._autoCloseTimer);
    sidebar._autoCloseTimer = null;
  }
  if (sidebar.classList.contains('translate-x-full')) {
    try { toggle.click(); } catch (_e) { /* noop */ }
  }
}
