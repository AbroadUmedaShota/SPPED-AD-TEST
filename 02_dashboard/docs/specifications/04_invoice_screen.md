...
- **目的**: ユーザーが請求内容の詳細を確認し、必要に応じてPDFや印刷形式で出力できるようにする。
- **レイアウト**: `seikyuusyo_sample.pdf` に準拠した、ロゴや社判を含む帳票形式のレイアウト。
- **表示項目**:
    - 請求書番号
    - 発行日
    - 支払期限
    ...
    - 発行元情報（ロゴ、社判、会社情報）
    - **請求明細**:
        - `invoices.json` のデータに加え、PDFサンプルに合わせた詳細なモックデータを表示（品名2, 数量, 単価など）
    - 小計、消費税、合計請求額
    - 振込先情報
- **操作**:
    - **印刷ボタン**: クリック時、`invoice-print.html` を新しいウィンドウで開き、印刷ダイアログを表示する。この画面はPDFと同じレイアウトを持つ。
    - **PDFダウンロードボタン**: クリック時、`html2pdf.js` を利用して、現在の画面 (`invoice-content` 部分) を `seikyuusyo_sample.pdf` と同じ見た目のPDFとして生成し、ダウンロードさせる。
