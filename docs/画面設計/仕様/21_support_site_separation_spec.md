...
- `/plans/` のファーストビューは、Standard / Premium の主要差分を2カードと少数の差分項目で先に提示する。無料プラン、速度別単価、追加料金、確認事項は後段の詳細比較として扱い、初見で全条件を読ませる構成にしない。
...
- ログイン前トップの導入事例ティザーも、ルート `data/customer-voices.json` ではなく `05_support/assets/data/customer-voices.json` を参照する。直接 `https://support.speed-ad.com/...` をfetchする方式は採用しない。
- 導入事例JSONの取得は5秒タイムアウト、600ms間隔で1回だけ自動再試行する。最終失敗時は読み込み中表示を残さず、企業名・担当者名・個別引用・個別画像を出さない汎用導線へ切り替える。
