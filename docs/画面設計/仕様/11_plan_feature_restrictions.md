...
## 0. 本書の境界
- 本書は `planTier` ごとの機能制限と UI/API の制御方針を扱う補助資料です。
- 価格、請求ポリシー、キャンペーン適用条件、契約/法務判断は shared の正本に含めません。
...
## 2. 対象プランと識別キー
| プラン表示名 | システムキー (`planTier`) | 主な特徴 | 備考 |
| :-- | :-- | :-- | :-- |
| Free (無料) | `free` | 基本アンケート、回答テキストのみDL、無料範囲内での名刺データ化。 | 期間30日、ロゴ表示あり。 |
| Premium (月額) | `premium` | 画像/名刺結合データDL、CSV出力、自社ドメイン、保存期間無期限。 | 2026-05-18 展開予定。 |
| Enterprise | `enterprise` | Premium＋SSO対応、SLA、高度な外部連携（Slack/CRM等）。 | 2026-07 以降（見直し対象）。 |

- `planTier` は UI/サービス/保存API で共通利用する判定キー。請求情報の `plan.displayName` とは別に保持する。
- `Premium` プランは 2026-05-18 のサービス展開に向けた主力プラン。

## 3. 機能カテゴリ別の制限マトリクス
| 機能カテゴリ | Free | Premium | UI制御方針 (Abroad) | 保存/API制御方針 (Rep) | 備考 | 更新日 |
| :-- | :-- | :-- | :-- | :-- | :-- | :-- |
| ダウンロード | 利用可 | 利用可 | P-09 はプレミアムプラン対象外のため、プラン差分としては扱わない。 | `downloadOptions` の認可チェック対象外とする。 | P-09 はプレミアムプラン対象外。モック画面は現状維持 | 2026-03-25 |
| グラフ分析出力 | 閲覧のみ | **CSV出力** 可能 | 出力ボタンに鍵アイコン＋ポップアップ。 | エクスポートAPIの権限検証。 |  |  |
