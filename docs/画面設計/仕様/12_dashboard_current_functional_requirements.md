...
- 共通サンプル `sv_0003_26010` は `SPEED ADアンケートサンプル` として表示し、会期終了・データ化完了済み・回答 10 件・名刺 10 件の標準機能確認用データとして扱います。会期はサービスリリース日に合わせ、2026-06-01〜2026-06-15 とします。
...
- URL に `surveyId` が無い新規作成時は、`data/surveys/templates/default-new-survey.json` を読み込み、アンケート名、表示タイトル、説明文、会期（明日から 3 日間）、サンプル設問 4 問を初期値として入れます。これらは通常入力と同じく編集・削除できます。
