...
## 対象範囲
- 本資料は、2026-06-19 時点の `02_dashboard` 配下に実装済みの挙動を整理したものです。
...
- `isSample: true` かつ `sampleVisibility: "all_accounts"` の共通サンプルは、サイドバーの表示単位（個人、営業、マーケティング、BPO）を問わず 1 件だけ表示します。
- 共通サンプル `sv_0003_26010` は `SPEED AD サンプルアンケート（標準機能確認用）` として表示し、会期終了・データ化完了済み・回答 10 件・名刺 10 件の標準機能確認用データとして扱います。会期はサービスリリース日に合わせ、2026-06-01〜2026-06-15 とします。
- 共通サンプルは `readOnly: true` のため、一覧では編集ボタンを無効状態にし、詳細・作成画面では編集、削除、保存を無効化します。詳細、QR、SPEEDレビュー、ダウンロード確認、複製は利用できます。複製後はサンプル属性を外し、通常アンケートとして扱います。

## アンケート作成（`surveyCreation.html`, `surveyCreation.js`, `surveyCreation-v2.js`）
- `surveyCreation.js` は v2 実装への 1 行エントリで、実装本体は `surveyCreation-v2.js` です。
- URL に `surveyId` が無い新規作成時は、`data/surveys/templates/default-new-survey.json` を読み込み、アンケート名、表示タイトル、説明文、会期（明日から 7 日間）、サンプル設問 4 問を初期値として入れます。これらは通常入力と同じく編集・削除できます。
- URL パラメータの `surveyName` / `displayTitle` / `periodStart` / `periodEnd` はテンプレートより優先されます。`surveyId` 指定の既存アンケート編集ではテンプレートを適用しません。
- 作成UIは日本語表示を基本としつつ、日本語・英語・中国語（繁体/簡体）・ベトナム語を扱う多言語作成に対応します。最大3言語を有効化可能です。
- 設問はフラットなカードとして追加・複製・並べ替え（Sortable）・削除でき、各設問タイプは型変更をサポートします。
- 対応設問は自由記述、単一選択、複数選択、ドロップダウン、数値、マトリクス（単一/複数）、日時、手書き、説明です。メタデータ編集で選択肢、行列、数値範囲、日時制約、手書きサイズ、表示制御を扱います。
- アウトラインサイドバーはジャンプリンク、QRモーダル、プレビュー、保存導線を提供します。
- Bizcard、サンクスメール、サンクス画面設定へのリンクは `surveyId` を引き継ぎ、保存は現状トースト表示のみです。
- 読取専用サンプルを作成画面で開いた場合は、保存・設問追加・削除・設問複製・型変更・入力変更を無効化し、プレビューと QR は確認用に利用できます。通常アンケートとして使う場合は、ダッシュボード詳細から複製します。
...
- 共通サンプル `sv_0003_26010` は回答 10 件・名刺 10 件を読み込み、標準的な検索、設問表示、詳細確認の動作確認に利用できます。
...
- 共通サンプルは `bypassDownloadDeadline: true` により、標準機能確認用としてダウンロード期限後でもダウンロード確認モーダルまで進めます。実ダウンロード生成はモック未実装です。
...
- メール送信、実ダウンロード生成、ライフサイクル更新はトースト/ログによる疑似挙動です。
