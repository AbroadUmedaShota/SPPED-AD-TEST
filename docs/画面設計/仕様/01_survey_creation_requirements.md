...
> **TL;DR（この文書の位置づけ）**
> - 対象は `02_dashboard/surveyCreation.html` と実装本体 `02_dashboard/src/surveyCreation-v2.js`（v2 系統）。`02_dashboard/src/surveyCreation.js` は同ファイルへの 1 行エントリ（`import './surveyCreation-v2.js';`）に過ぎない。
> - 本書は「現行実装のトレースドキュメント」。HTML / JS の実装を根拠に書き、旧仕様書群（`01_survey_creation_*` / `02_survey_creation*` 系 7 本）と実装が食い違っていた箇所は実装側に合わせて整合済み。
> - 新規作成時は `data/surveys/templates/default-new-survey.json` を使い、管理名・表示タイトル・会期・説明文・サンプル設問 4 問を編集可能な初期値として入れる。既存アンケート編集と URL 指定値は上書きしない。
...
- `data/surveys/templates/default-new-survey.json`（新規作成時の初期値テンプレート）
...
- v3.1 (2026-06-19): 新規作成初期値テンプレート、共通サンプルアンケートの読取専用制御、`surveyCreation.js` の v2 入口統一を反映。
...
- `surveyId` 指定の既存アンケート編集ではテンプレートを適用しない。
...
**読取専用サンプルの制御**:
- 読み込んだアンケートに `readOnly: true`、または `isSample: true` + `sampleVisibility: "all_accounts"` がある場合、作成画面は読取専用モードに入る。
- 画面上部に標準機能確認用サンプルである旨を表示し、保存・設問追加・設問削除・設問複製・設問タイプ変更・必須切替・入力変更を無効化する。
- プレビューと QR 表示は確認用として利用できる。
- サンプルを編集可能な通常アンケートとして利用する場合は、ダッシュボードの詳細モーダルからアンケート複製を行う。
...
| `surveyId` | `document.body.dataset.currentSurveyId` に保持 + `loadSurveyData(surveyId)` 呼出 + 関連設定リンク URL を `buildRelatedSettingsUrl` で再構築。読取専用サンプルの場合は編集操作を無効化 |
...
`surveyId` が無い場合は、URL パラメータ復元後に新規作成テンプレートを適用する。テンプレートは空欄だけを補完するため、URL 指定値やユーザー操作済みの値は維持される。
...
7. **新規作成テンプレート適用** — `surveyId` が無い場合のみ、空欄と空の設問リストに初期値を補完。
8. **初期描画** — `renderLangSelectionAndTabs()` / `updateOutline()` / `updateEmptyState()`。
...
- [ ] 新規作成時にテンプレート初期値（管理名・表示タイトル・説明文・会期・設問 4 問）が入る
- [ ] URL 指定値および `surveyId` 指定の既存編集がテンプレートで上書きされない
- [ ] 読取専用サンプルでは保存・編集系操作が無効化され、プレビュー・QR は利用できる
