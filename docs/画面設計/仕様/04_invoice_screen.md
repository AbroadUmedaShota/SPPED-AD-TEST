...
`プレミアム利用料` `追加アカウント料金` `従量課金` は、同一請求書内で合算される場合でも別行で表示する。`クーポン値引き` を表示する場合は、対象課金の値引きとして独立行で表示する。

品目1 / 品目2 の表示ルールは以下とする。

- アンケート非依存の行:
  - 品目1 = 課金種別
  - 品目2 = 対象月、対象組織名、適用条件などの詳細。追加アカウント料金は `全アカウント数 / 無料アカウント数 / 請求対象アカウント数` が分かる表現を優先する
- アンケート依存の行:
  - 品目1 = アンケート名
  - 品目2 = `名刺データ化費用` `お礼メール送信費用` `クーポン値引き` などの課金種別とその根拠
- 小計行を表示する場合:
...
- 明細が多い場合はページを自動追加する。
  ...
  - 1ページ目は14行相当を安全側の目安とし、実際の表示件数は `品名1` `品名2` の折り返し量に応じて増減する
...
- `AGG-...` の集計請求では、上記に加えて元請求書単位の `請求書小計` を表示する。
- 明細の並びは `プレミアム利用料` → `追加アカウント料金` → 契約系小計 → アンケートごとの明細と小計 → `請求書小計`（集計請求時のみ）とする。
- 明細表の `品名1` は品目1、`品名2` は品目2として扱う。
- `品名1` はアンケート依存の行ではアンケート名、アンケート非依存の行では課金種別を表示する。
- `品名2` は課金種別の詳細、対象月、適用プラン、無料枠超過、値引き理由などを表示する。
...
- `AGG-...` の集計請求では、各請求書ブロックの末尾に `請求書小計` を表示する。
...
- INV-003: `id` に応じた内容が表示され、契約系小計・アンケート小計・請求書小計（集計請求時）を含めて印刷プレビューで崩れない。
