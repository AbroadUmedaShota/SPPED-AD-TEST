...
### 1.4 利用者向け請求書仕様との境界
- 本書は `03_admin/billing-management.html` の管理者向け請求管理 UI を扱う。
- 利用者向け請求書ページ（INV-001/002/003）の正本は `docs/画面設計/仕様/請求関連仕様マップ.md` を入口とし、表示/帳票要件は `04_invoice_screen.md` と `05_invoice_document.md` を参照する。
- 管理者画面の `PDFプレビュー` 等の表現が利用者向け帳票を参照する場合でも、本書では管理者側の操作責務と導線だけを定義する。
