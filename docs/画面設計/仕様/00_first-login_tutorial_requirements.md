...
本番環境での独立ページ配置は `https://support.speed-ad.com/tutorial/` を正本とする。従来の `04_first-login/index.html` は移行元として扱い、サポートサブドメイン上の静的な初回操作案内・再視聴導線へ整理する。ログイン後アプリ側に残す場合は、初回判定や `/tutorial/` への導線など、動的機能に必要な最小要素に限定する。
...
| :--- | :--- | :--- | :--- |
| F-001 | チュートリアル自動開始 | 新規作成されたアカウントが初回ログインした際、必要に応じて初回操作案内へ誘導する。 | ・初回ログイン判定は、ユーザーオブジェクトに `isFirstLogin` や `lastLoginAt` のようなフラグを持たせ、サーバーサイドで判定するのが望ましい。<br>・本番の独立ページは `https://support.speed-ad.com/tutorial/` とし、アプリ側は表示バナー、案内モーダル、またはリンク導線として扱う。 |
| F-002 | スキップ機能 | ユーザーは任意のタイミングで初回案内を閉じ、メインのダッシュボード画面へ進むことができる。 | ・アプリ内に案内バナーまたはモーダルを出す場合、スキップ後はダッシュボード画面 (`/02_dashboard/index.html`) を継続表示する。<br>・スキップ状態を保存する場合は、サーバーサイドの初回案内状態を正とし、`localStorage` は補助に留める。 |
| F-003 | チュートリアルの再実行 | 一度完了またはスキップした後でも、ユーザーが任意に初回操作案内を再確認できる導線を設ける。 | ・サイドバーまたはヘルプ導線に「初回チュートリアルをもう一度見る」リンクを配置する。<br>・クリックすると `https://support.speed-ad.com/tutorial/` に遷移する。 |
...
graph TD
    A[ログイン] --> B{初回ログインか？};
    B -- Yes --> C[初回操作案内<br>https://support.speed-ad.com/tutorial/];
...
- **本番配置:**
  - 静的な説明ページ、動画、キャプチャ、手順記事への導線は `https://support.speed-ad.com/tutorial/` に集約する。
  - アプリ内で実際の入力や保存を伴うトレーニングを残す場合、その部分は `app.speed-ad.com` 側の動的機能として別途仕様化する。
  - 旧 `04_first-login/index.html` は本番公開時に `/tutorial/` へリダイレクトする。
