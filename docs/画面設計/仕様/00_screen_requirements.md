...
- `screen_code`: `SCR-U-001`, `SCR-U-008`-`SCR-U-014`, `SCR-U-026`-`SCR-U-033`, `SCR-M-001`-`SCR-M-010`, `SCR-M-012`-`SCR-M-019`
- `related_feature_codes`: `FEAT-004`, `FEAT-005`, `FEAT-016`-`FEAT-023`, `FEAT-030`-`FEAT-033`, `FEAT-053`-`FEAT-060`
...
- **行アクション（各行の操作列）**:
    - **編集**: アンケート編集画面 `02_dashboard/surveyCreation.html?surveyId={id}&surveyName={encodedName}` へ遷移する（実装: `02_dashboard/src/tableManager.js` の `button[title="アンケートを編集"]` クリックハンドラ）。`surveyName` は `encodeURIComponent` でエンコードした表示名を引き継ぐ。
        - 共通サンプル（`isSample: true` かつ `sampleVisibility: "all_accounts"`、または `readOnly: true`）は編集不可とし、編集ボタンは無効状態で「サンプルのため編集できません」と理由を示す。
...
- **編集可否ルール**: ステータスが「会期前」の場合のみ、以下のフィールドが編集可能。それ以外は読み取り専用とし、`disabled` 属性を付与、視覚的にグレーアウト、ヘルプアイコン（`?`）のホバー／フォーカスで理由をツールチップ（Material Tooltip）で表示する。共通サンプルはステータスに関わらず読取専用とし、編集・削除は表示しない。
...
- **UI 補足**:
    ...
    - 共通サンプルでは「標準機能確認用のサンプルです。編集・削除はできません。複製して通常アンケートとして利用できます。」と案内し、複製導線は残す。
...
- **共通サンプルの扱い**: 共通サンプルを複製した場合は、現在選択中の表示単位に紐づく通常アンケートとして作成し、`isSample` / `sampleVisibility` / `readOnly` などのサンプル属性は引き継がない。回答数・名刺データ化完了数・データ化完了状態は 0 / 未完了にリセットする。
- **閉じ方**: §5.5.0 共通仕様に従う。

#### 5.5.9. プレビューモーダル（`surveyPreviewModal`）

- **開き方**: アンケート作成・編集画面のプレビューボタン。
- **主な機能**: スマートフォン／タブレットの 2 デバイスで切替表示。iframe で実回答ページをサンドボックス読み込みし「プレビューモード - 回答データは送信されません」を上部バナーで明示する。
- **閉じ方**: §5.5.0 共通仕様に従う。
...
- **回答フォーム**: 設問種別に応じた入力コントロールを表示する。サポートする設問種別は以下の通り（`02_dashboard/src/surveyCreation.js` の `QUESTION_TYPES` 定義および `data/core/surveys.json` の `type` 値を正典とする）。
...
#### 5.10.8. アンケート作成・編集（`surveyCreation.html`）

- 新規アンケート作成モーダル（§5.5.1）または編集行アクション（§5.4.3）から遷移する作成・編集ページ。現行 UI で、設問追加・並べ替え・条件分岐などの編集体験を提供する。
...
| バージョン | 日付 | 変更概要 |
|:---|:---|:---|
| 2.4.1 | 2026-06-20 | 共通サンプルアンケートの操作仕様を追記。一覧の編集ボタンは無効状態で理由を示し、詳細では編集・削除を出さず、複製後はサンプル属性を外した通常アンケートとして扱うことを明記。 |
