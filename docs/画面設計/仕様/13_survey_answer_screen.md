...
画面は「メインコンテンツ」「フッター」の2つの主要エリアで構成され、独立したヘッダー領域およびサイドバーは設けない。UIコンポーネントは、プロジェクトの共通デザインガイドライン (`docs/プロダクト/UIデザイン/00_design_guideline.md`) に準拠すること。

-   **メインコンテンツ**
    -   **言語選択ドロップダウン**: プレミアムプランの多言語対応アンケートの場合、メインコンテンツの最上部に言語選択用のドロップダウンリストを右寄せで表示する。
    -   アンケートのタイトルと説明文を表示する。
    -   アンケートの設問を上から順に一覧表示する。
    -   現時点では、ページネーションによる設問の分割表示は行わない（※将来実装の可能性あり）。
    -   **名刺プレビューエリア**: 名刺撮影後、設問一覧の下・フッターの上に撮影済み名刺（表面・裏面）のプレビューを表示する。
-   **フッター**
    -   ページの**最下部**に固定配置される。
    -   上段に以下の2つのボタンを横並び（グリッド）で配置する。
        1.  **名刺画像撮影ボタン**: 名刺をカメラで撮影する機能を起動する。
        2.  **アンケート送信ボタン**: 全ての回答を送信する。
    -   下段に **名刺が手元に無い方向けリンク** をテキストリンク形式で配置する。押下すると名刺情報を手動で入力するためのモーダルを開く。
...
6.  **画面遷移**: 保存後、`thankYouScreenSettings.html` で設定された内容に基づくサンクス画面へ自動的に遷移する。

### 3.2. 回答のドラフト保存機能

-   **手動保存**: 現時点では専用の一時保存ボタンは設けない。自動保存のみで対応する（※将来実装の可能性あり）。
...
**名刺入力言語の初期値と優先順位**
- 初期値はアンケート作成時に設定された優先回答言語（`defaultAnswerLocale`）。回答画面の言語ドロップダウンで上書きした場合はその値を採用する。
- OCR結果は補助情報として表示し、入力言語の決定は上記の優先順を維持する。
...
-   **フォールバック**: 言語の判別が不可能だった場合は、現在アンケートが表示されている言語（初期値は優先回答言語）をデフォルトの言語として表示する。
...
設問種別に応じて、適切な入力コントロールを表示する。データモデルのキーは `docs/プロダクト/アーキテクチャ/02_data_model.md` の定義と整合性を保つこと。
