...
画面は「メインコンテンツ」「フッター」の2つの主要エリアで構成され、独立したヘッダー領域およびサイドバーは設けない。UIコンポーネントは、shared repo の既存画面パターンと共通コンポーネントに準拠すること。
...
設問種別に応じて、適切な入力コントロールを表示する。データモデルのキーは本書および既存のアンケート作成/回答実装と整合性を保つこと。
