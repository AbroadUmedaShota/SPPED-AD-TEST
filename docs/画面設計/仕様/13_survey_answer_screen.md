...
owner: product
contact:
  team: SpeedAd 製品企画チーム
  escalation: operation@abroad-o.com
impl_ref:
  html: 02_dashboard/survey-answer.html
  js: 02_dashboard/src/survey-answer.js
  snapshot_date: 2026-04-17
---

# 13. アンケート回答画面 要件定義書

**本書はモックアップ定義書である。** ここで言うモックアップとは、画面側（HTML / JS）の現状挙動を正として仕様を文書化したものであり、本番実装に向けた要件定義の一次資料として位置付ける。現行フェーズではバックエンド API は未接続であり、回答データ・ドラフト・手入力による名刺情報は `localStorage` を暫定ストレージとして利用している。提出先の開発会社は、実装上の制約および今後の対応予定を把握するため、本書の §6（未実装項目）および §9（変更履歴）を必ず参照すること。実装の参照起点は [`02_dashboard/survey-answer.html`](../../../02_dashboard/survey-answer.html) および [`02_dashboard/src/survey-answer.js`](../../../02_dashboard/src/survey-answer.js) とし、仕様変更時は実装と本書を併せて更新すること。

## 1. 概要

### 1.1 目的
本書は、イベント来場者・被調査者が SpeedAd の配信URLからアクセスして回答を行う「アンケート回答画面」の要件を定義する。モックアップとして実装済みの `02_dashboard/survey-answer.html` および `02_dashboard/src/survey-answer.js` を正とし、画面構造・UI挙動・モーダル遷移・ドラフト保存仕様を、実装と同期した単一の参照資料として整備することを目的とする。
...
| 開発委託先 | 本番実装に向けた要件一次資料。§6（未実装項目）および §9（変更履歴）を起点に差分開発を行う。 |
...
> **本契約スコープ外であることの宣言**
> 本章に列挙する項目は、**本契約（モックアップ納品）の成果物に含めない**。以降の機能追加フェーズまたは別途オプション契約での対応対象とする。各項目の工数・要件定義は別途協議の上で確定する。
...
| 1.0.0 | 2026-04-17 | 初版。実装モックアップを正として定義書を全面整備 | 製品企画 |
