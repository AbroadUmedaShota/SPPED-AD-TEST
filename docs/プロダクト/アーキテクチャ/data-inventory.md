...
- `speed-review.js`: combines `data/core/surveys.json`, `data/responses/answers/<surveyId>.json`, `data/responses/business-cards/<surveyId>.json`, and demo payloads in `docs/サンプル/demo_surveys/<surveyId>.json`.
...
- `graph-page.js`: consumes `docs/サンプル/demo_surveys/<id>.json` and `.../demo_answers/<id>.json`.
...
- `docs/サンプル/demo_surveys/`: demo survey definitions for charts and training data（`resolveDemoDataPath('surveys/...')`で解決）。
- `docs/サンプル/demo_answers/`: sample answer exports tied to demo surveys.
- `docs/サンプル/demo_business-cards/`: persona data linked to demo answers.
- `アーカイブ/data-dumps/`: archived heavy exports (`0008000154*.json`, CSV dumps).
## Follow-ups
...
- Demo data: moved to `docs/サンプル/demo_*` and resolved via `resolveDemoDataPath`; audit remaining modules for direct string paths.
