...
*   **請求書一覧・詳細機能:**
    *   **一覧画面:** 個別の請求書ではなく、**請求月および契約種別（個人／グループ）ごと**に集計された請求サマリーをカード形式で表示する。各カードには、合計請求額、請求期間、種別などが表示される。
    *   **詳細画面:** 一覧から特定の請求サマリーを選択すると、その内訳となる個別の請求書詳細を確認できる。
...
本フェーズでは、`docs/プロダクト/アーキテクチャ/02_data_model.md` に定義されたデータ構造に基づき、`data/` ディレクトリ内に配置された静的なJSONファイル（例: `surveys.json`, `invoices.json`）をデータソースとして使用します。
...
本システムの商用利用時における料金体系および請求計算のロジックを以下に定義します。
...
**8.2. 従量課金要素**
以下の項目については、利用実績に応じた従量課金が発生します。
*   **名刺データ化費用:** 展示会で取得した名刺のデータ化件数に応じた費用（単価は選択したスピードプランに準ずる）。
*   **お礼メール送信費用:** システム経由で送信されたお礼メールの総通数に応じた費用。

**8.3. グループ追加アカウント料金**
グループ機能利用時、所属人数に応じて以下の追加料金が発生します。
*   **2名まで（管理者含む）:** 無料。
*   **3名以上:** 1名追加につき、月額 300円（税別）の追加料金。

**8.4. 請求の取りまとめ**
*   グループのアカウント料、およびグループ内のアンケートに関連するすべての費用（データ化、メール送信等）は、**グループの管理者**に対して一括して請求されます。

### 9. サービス品質保証 (SLA)

本サービスでは、リード情報の鮮度を保つため、以下のデータ化目標時間を掲げています。

**9.1. データ化の目標時間**
*   **対象:** アンケート回答および名刺送信が行われたデータ。
*   **目標値:** 送信完了から **1時間以内** のデータ化完了を目指します。
*   **運用:** 当面は現状の体制（日本時間 9:00〜17:00 等の制限有無は今後検討）でこの目標値を維持し、将来的な海外展開に合わせて24時間体制への拡張を検討します。
