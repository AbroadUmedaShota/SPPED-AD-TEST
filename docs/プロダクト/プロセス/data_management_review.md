...
## 1. 参照ドキュメント
- ステータス草案: `docs/変更履歴/status_draft.md`
- ダッシュボード実装方針: `docs/画面設計/仕様/12_dashboard_current_functional_requirements.md`
  - 画面で見せるステータスを正規化し、削除済みは除外している
  - ライフサイクルに応じてダウンロード可否を判定している
- 画面要件: `docs/画面設計/仕様/00_screen_requirements.md`
  - `削除済み` / `不明` はフィルタ選択肢に出さない
- 権限制御とステータス挙動: `docs/ja/user-permissions.md`
- データモデル: `docs/プロダクト/アーキテクチャ/02_data_model.md`
- 多言語名刺フロー（保管ステップあり）: `docs/プロダクト/プロセス/multilingualBizcardFlow.md`
- プラン別保存期間: `docs/リファレンス/リソース/client-materials/service-plan-comparison.md`（無料/Standard: 30日、Premium: 無期限）
...
- ダッシュボード「アンケート一覧」 (`docs/画面設計/仕様/00_screen_requirements.md`)  
  - フィルタ・表示: 会期前 / 会期中 / データ精査中 / 完了 / 終了 / データ化なし （削除済み・不明は非表示）
- ダッシュボード「請求（請求一覧・詳細）」 (`docs/画面設計/仕様/04_invoice_screen.md`)  
  - ステータス: 未入金 / 入金済 / 延滞 / 取消 （フィルタの初期値「すべて」を含む）
- グループ編集（メンバー管理） (`docs/画面設計/仕様/group_edit_requirements.md`)  
  - バッジ並び順: グループ加入済 / グループ招待中 / アドレスエラー
- 管理画面「クーポン管理」 (`docs/画面設計/仕様/admin/coupon_management_requirements.md`)  
  - プルダウン: すべて / 有効 / 利用上限到達 / 利用停止 / 期限切れ
- 管理画面「カレンダー管理」 (`docs/画面設計/仕様/admin/calendar-management_requirements.md`)  
