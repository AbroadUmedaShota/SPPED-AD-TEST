...
## 参照ドキュメント
- `docs/プロダクト/プロセス/multilingualBizcardFlow.md`（既存フロー全体と不一致ハンドリング）
- `docs/画面設計/仕様/02_survey_creation.md:211-227`（多言語回答フローの不一致・再割当分岐）
- `docs/プロダクト/アーキテクチャ/02_data_model.md:238-361`（`input_business_cards` の `language_code` 整合性チェック）
- `docs/画面設計/仕様/admin/00_admin_requirements_design.md:36-106`（言語別キュー管理と再割当要件）
