...
## A. アンケート End-to-End フロー
**ペルソナ**: 管理者, オペレーター, 顧客/回答者

```mermaid
flowchart TD
    subgraph ADMIN ["管理者 / 利用者"]
        A1["1. 要件整理 / 設問設計"]
        A2["4. 最終承認 / 公開"]
    end

    subgraph OPR ["オペレーター"]
        O1["2. アンケート作成 (surveyCreation.html)"]
        O2["3. プレビュー / 調整"]
        O3["6. 回答モニタリング (speed-review.html)"]
        O4["7. レビュー / 差戻し"]
        O5["8. データエクスポート"]
    end

    subgraph CUST ["顧客 / 回答者"]
        C1["5. フォーム入力 / 送信"]
    end
...
```mermaid
flowchart TD
    subgraph OPR_B ["オペレーター"]
        B1["1. 請求データ入力 (invoiceDetail.html)"]
        B2["4. 入金確認 / 消込"]
    end

    subgraph MGMT_B ["入力管理者"]
        OA1["2. 入力内容レビュー"]
    end

    subgraph ADMIN_B ["管理者"]
        AD1["3. 最終承認 / 発行"]
        AD2["5. 督促 / 例外処理"]
    end
...
flowchart TD
    ...
    subgraph DEV_C ["開発 / 保守担当"]
        D01["5. 技術調査 / 恒久対策"]
    end

    subgraph OPR_C ["オペレーター"]
        OX1["調査補助 (データ提供など)"]
    end
...
```mermaid
flowchart TD
    subgraph OPR_D ["オペレーター"]
        D1["1. タスク確認"]
        D2["2. データ入力 (data_entry.html)"]
        D3["3. 突合処理 (reconciliation)"]
        D4{4. 差異あり？}
        D6["5b. 進捗確認"]
    end
    
    subgraph MGMT_D ["入力管理者"]
        D5["5a. 差異の確認と指示"]
        D7["6. 全体進捗の監督"]
    end
