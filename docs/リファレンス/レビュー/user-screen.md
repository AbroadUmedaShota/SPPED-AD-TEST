...
- **内容**: `initializePage` はデモ用 `docs/サンプル/demo_surveys` に設問定義がない場合、`data/surveys/enquete/<surveyId>.json` へフォールバック取得していますが、取得結果を `currentSurvey.details` に反映する際に元の `enqueteDetails` 変数を参照しています。そのためフォールバックが成功しても `currentSurvey.details` が空配列のままとなり、設問ヘッダーや回答絞り込みが利用できません。
