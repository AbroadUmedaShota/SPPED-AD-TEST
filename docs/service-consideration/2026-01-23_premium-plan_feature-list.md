...
補足：既存のプラン比較表には 90日 記載があるため、**差分がある（要整合）**。
...
### リリーススコープ（THK社向け）
- **2026-04-02（先行リリース）**：プレミアム販売体制は未整備でも、THK社が必要とする機能（手書き設問など）を内部ロジックとして先行実装
