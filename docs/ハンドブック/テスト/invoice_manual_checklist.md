...
4. 契約種別バッジ（`個人` / `グループ`）
5. 請求先アカウント名（例: `個人アカウント` `営業部 (組織)`）
6. 請求先
7. 合計請求額（税込、3桁区切り）

### 3.3 遷移

1. カードをクリックすると `invoice-detail.html?id=INV-...` に遷移すること。
...
4. 明細に `プレミアム利用料` `追加アカウント料金` `名刺データ化費用` `お礼メール送信費用` `クーポン値引き` の主要粒度が、必要に応じて別行で判別できること。
5. グループ請求書（例: `INV-2025-11-003` `INV-2025-11-004`）では、宛名が対象グループの請求先として解決されること。
6. 明細数が多い請求書では、1ページ目が不自然に早く打ち切られず、実表示高さに収まる範囲まで表示されること。
7. 同一アンケート内で `名刺データ化費用` が `お礼メール送信費用` より先に表示されること。
8. `お礼メール送信費用` 行がある場合、`description` から `送信対象 N件 / 無料100件控除` が読み取れること。
...
3. 明細に `プレミアム利用料` `追加アカウント料金` `名刺データ化費用` `お礼メール送信費用` `クーポン値引き` の主要粒度が、期待どおりの行として表示されること。
4. `クーポン値引き` が存在する場合、対象課金に埋もれず独立行として表示されること。
5. 同一アンケート内で `名刺データ化費用` が `お礼メール送信費用` より先に表示されること。
6. `お礼メール送信費用` 行がある場合、`description` から `送信対象 N件 / 無料100件控除` が読み取れること。
...
### 5.4 集計ID（`AGG-...`）の内部互換確認（任意）

1. `invoice-detail.html?id=AGG-...` で直接アクセスした場合でもエラーにならず表示できること。
2. `invoice-detail.html?id=AGG-...` で表示される「小計」行（`isSubtotal: true`）が、`invoice-print.html?id=AGG-...` では出力されないこと（現行仕様）。
...
3. `items[]` の明細名または同等の表現が、`プレミアム利用料` `追加アカウント料金` `名刺データ化費用` `お礼メール送信費用` `クーポン値引き` の主要粒度に沿っていること。
4. `クーポン値引き` を表示する場合、独立した明細行として構成されていること。
5. `お礼メール送信費用` 行がある場合、`description` に記載された送信対象件数が、対応する `名刺データ化費用.quantity` を超えていないこと。
6. `お礼メール送信費用` 行がある場合、`quantity = 送信対象件数 - 100` になっていること。
7. `名刺データ化費用.quantity <= 100` のアンケートで、`お礼メール送信費用` 行が存在しないこと。
