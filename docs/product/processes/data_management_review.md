...
- プラン別保存期間: `docs/references/resources/client-materials/service-plan-comparison.md`（無料/Standard: 90日程度の注記あり、Premium: 無期限）
...
### 3.3 保持期間のたたき台
- プラン別のベースライン  
  - 無料/Standard: 90日を目安（会期終了またはデータ化完了を起点）。  
...
- 回答データ（本文・選択値・タイムスタンプ）: プラン基準（無料/Standard 90日、Premium 無期限）。法務要件があれば延長。
- 名刺データ化（画像原本）: 短期保持（30〜60日）を上限とし、抽出後は削除または匿名化。バックアップも同じ期間でローテーション。
- 名刺データ化（構造化データ）: プラン基準で保持。会期終了/データ化完了を起点に90日（無料/Standard）を案とする。
...
- プラン別の正式な保持日数を確定する（90日を採用するか、法務確認を含め決める）。
