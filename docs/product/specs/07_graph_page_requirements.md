...
- **Excel一括出力:**
    ...
    - **各設問1シート構成**: シート名には設問文を使用。
    - **ハイブリッド出力**:
        - **左側**: Excelの正式な「テーブル機能」による集計表（フィルタ・ソート可能）。
        - **右側**: 高解像度（scale: 3）でキャプチャされたグラフ画像を埋め込み。
    - **軽量・高編集性**: 画像データとテーブルデータを適切に配置し、報告資料としてそのまま活用可能な品質を実現。

## 4. ユーザーインターフェース (UI) 要件

- **ページレイアウト:**
    - 集計カード内は「左側に集計テーブル、右側にグラフ」の構成とし、数値と視覚データをセットで確認しやすくする。
- **フィルタエリア:**
    - 左側に検索タブ（簡易・詳細）、右側に表示設定パネルを配置。
- **グラフ上の数値表示:**
    - `chartjs-plugin-datalabels` を使用し、円グラフにはパーセンテージ、棒グラフには件数を直接描画する。
- **視認性向上:**
    - グラフタイトル横に、設問特性に応じたアイコンを表示。
        - `analytics`: 集計・分析可能な設問
        - `subject`: テキスト回答主体の設問（グラフ化対象外）
