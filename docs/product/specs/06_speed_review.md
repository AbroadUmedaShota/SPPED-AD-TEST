...
- **配置**: データテーブルの上部に配置。
- **構成要素**:
    1.  **KPIカードエリア**: 3つの主要指標を表示するカード群。
        - **回答総数 (Total Answers)**: フィルタリングされた現在の総回答数。
        - **最終回答 (Latest Activity)**: 直近の回答が「何分前」にあったかを表示（例: "5分前"）。
        - **現在選択中の設問 (Current Question)**: メインテーブルで表示中の動的な設問名を表示する。
    2.  **時間帯別トレンドグラフ (Time Series Chart)**:
        - **種類**: 棒グラフ (Bar Chart)。
        - **軸**: 横軸＝時間（0時〜23時）、縦軸＝回答数。
        - **特徴**: イベント等の「その日の動き」を可視化するため、当日の推移を表示。
    3.  **属性構成チャート (Attribute Chart)**:
        - **種類**: ドーナツグラフ (Doughnut Chart)。
        - **内容**: サイドバーで選択中の「設問（例：業種）」の回答内訳を表示。

### 3.5. 右サイドバー（ツールボックス）

- **目的**: 回答データのフィルタリング、検索、および表示設問の切り替え機能を提供する。
- **配置**: 画面右側にフローティングで表示。開閉可能。
- **構成要素**:
    1.  **キーワードで検索**: 全文検索用の入力フィールド。
    2.  **回答で絞り込み**: 選択中の設問の回答候補でデータを絞り込むドロップダウン。
    3.  **日時で絞り込み**: 日付/時刻範囲で回答を絞り込むための入力フィールド。
    4.  **フィルターをリセット**: 全てのフィルター条件を初期状態に戻すボタン。
    5.  **グラフ化**: 選択中の設問の分析ダッシュボード（アナリティクスダッシュボード）の表示/非表示を切り替えるボタン。
    6.  **表示設問**: テーブルの動的カラムに表示するアンケート設問を選択するためのラジオボタン群。
...
- **ダッシュボード連動**:
    - **フィルター連動**: キーワード検索や日付フィルターが適用された際、ダッシュボードの全ての数値・グラフも即座に再集計・再描画されること。
    - **設問選択連動**: サイドバーで表示対象の設問（カラム）を変更した際、属性構成チャートも該当する設問の集計結果に切り替わること。
