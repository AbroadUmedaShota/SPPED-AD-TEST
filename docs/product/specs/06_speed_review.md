...
- **構成要素**:
    1.  **KPIカードエリア**:
        - **Total Answers**: フィルタリングされた現在の総回答数。
        - **Latest Activity**: 直近の回答が「何分前」にあったかを表示（例: "最終回答: 5分前"）。ライブ感を演出する。
    2.  **時間帯別トレンドグラフ (Time Series Chart)**:
        - **種類**: 棒グラフ (Bar Chart)。
        - **軸**: 横軸＝時間（0時〜23時）、縦軸＝回答数。
        - **特徴**: イベント等の「その日の動き」を可視化するため、当日の推移を表示。ピークタイムがひと目で分かるデザインとする。
    3.  **属性構成チャート (Attribute Chart)**:
        - **種類**: ドーナツグラフ (Doughnut Chart)。
        - **内容**: サイドバーで選択中の「設問（例：業種）」の回答内訳を表示。
        - **特徴**: 真ん中に最大勢力のラベルを表示するなどして、省スペースで情報を伝える。
