...
- **目的**: 回答データのフィルタリングおよび表示設問の切り替え機能を提供する。
- **配置**: 画面右側にフローティングで表示。開閉可能。
- **構成要素**:
    1.  **日時で絞り込み**: 日付/時刻範囲で回答を絞り込むための入力フィールド。
    2.  **フィルターをリセット**: 絞り込み条件を初期状態に戻すボタン。
    3.  **グラフ化**: 選択中の設問の分析ダッシュボード（アナリティクスダッシュボード）の表示/非表示を切り替えるボタン。
    4.  **表示設問**: テーブルの動的カラムに表示するアンケート設問を選択するためのラジオボタン群。
