...
- **アナリティクスダッシュボード:**
    - **現在選択中の設問カード (`#kpi-current-question-card`):**
        ...
        - **UI**: ドロップダウン風のデザイン。右端に `arrow_drop_down` アイコンを配置し、エリア全体がクリッカブルであることを示唆。ホバー時に背景色が変化。
...
- **プランによる制限**: エクスポート可能なデータの種類は、利用者の契約プラン（Free / Premium）によって動的に制御される。

### 5.2. プラン別エクスポート権限マトリクス
| 出力データ種別 | Free (無料) | Premium (月額) | 備考 |
| :--- | :---: | :---: | :--- |
