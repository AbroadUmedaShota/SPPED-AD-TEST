...
### 1.2 背景
キャンペーン施策や新規顧客獲得のためのインセンティブとして、柔軟な条件でクーポンを発行・管理する必要がある。本要件定義書は、そのための画面仕様を明確化するものである。
...
- **検索項目:**
    - **キーワード検索:** `コードと名称を検索。入力がメールアドレス形式の場合、対象メールアドレスも検索対象に加える。`
...
### 3.2 クーポン一括作成（モーダル）
- 「一括作成」ボタンからモーダルで起動する。
- 外部で準備したメールアドレスリストに基づき、対象者ごとにユニークなクーポンを発行するプロセスを支援する。

#### ステップ1: 共通設定と対象者入力
- 作成するクーポンに共通の情報と、対象者のメールアドレスリストを入力する。
- **対象メールアドレス:** `必須`, `1行に1つのメールアドレス形式で入力するテキストエリア`。入力された行数が作成数となる。
- **割引率:** `必須`, `1〜50の整数`
- **有効期限:** `必須`
...
#### ステップ2: 個別設定
- ステップ1で入力されたメールアドレスの数だけ行を持つテーブルを表示する。
- **UI挙動:**
    - **対象メールアドレス（読み取り専用）:** ステップ1で入力したメールアドレスを表示。
    - **クーポンコード:** システムが自動生成したユニークなコードを予め入力しておく。ユーザーは手動で編集可能。
    - **クーポン名称:** ユーザーが自由に入力。入力がない場合は「プロモーションクーポン」のようなデフォルト名が設定される。
- **バリデーション:**
    - ユーザーが手動で編集したクーポンコードに対しては、既存コードおよび当バッチ内での重複チェック、形式チェック（半角英数字ハイフン）を行う。

#### ステップ3: 確認と作成
- **UI挙動:**
    - 共通設定の内容と、作成されるクーポン（対象メールアドレス、コード、名称）の一覧を最終確認画面として表示する。
    - 「作成実行」ボタンを押下すると、サーバーサイドで一括作成処理が実行される。
    - 作成成功時には「クーポンをXX件作成しました」というトーストメッセージを表示し、モーダルを閉じてクーポン一覧を更新する。
    - 作成失敗時には、どのクーポンで問題が発生したかを示すエラーメッセージをモーダル内に表示する。

#### ステップ4: CSVエクスポート
- 作成成功後、管理者がメール配信ツールで利用するために、結果をCSVファイルでダウンロードする機能を提供する。
- **UI挙動:**
    - 作成成功時のトーストメッセージに「CSVダウンロード」ボタンを含める、または作成完了画面にボタンを配置する。
    - ダウンロードされるCSVファイルには、最低限「メールアドレス」と「クーポンコード」のペアが含まれる。
...
- **編集可能項目:**
    ...
    - メモ
- **UI挙動:**
    - 編集不可項目（コード、割引率など）はフォーム内で非活性（disabled）表示とする。
