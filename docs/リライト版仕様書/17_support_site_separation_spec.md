...
| 項目 | 値 |
| --- | --- |
| 対象 | ダッシュボード（`app.speed-ad.com`）から `support.speed-ad.com` サブドメインへの静的ページ切り出し時の確認事項 |
| 正本区分 | 本書は補足メモ。開発会社提出時は、サポート分離の考え方を確認する参考資料として扱う |
| owner | product |
| last_reviewed | 2026-04-24 |

## 1. 文書目的

- ヘルプ・FAQ・規約などの静的ページを、ダッシュボードから `support.speed-ad.com` サブドメインへ切り出す画面外の確認事項を整理する。
- 本書は、サポート関連ページを分離する場合の確認事項を整理した補足資料である。
- KPI・ROI・予算承認・詳細スケジュールは本書スコープ外とする。法務合意・マーケ合意などは実装前提条件として別途取得する。

### 1.1 本番化要件の扱いに関する注記

- 本書は補足メモであるため、個別画面の本番実装要件は 13〜16 の各仕様書を優先する。
- 本番化に必要な要素は既に以下の既存セクションへ分散して記述済みである。実装着手時は、本書ではなく個別の PRD/ADR へ転記・詳細化することを前提とする。
  - §3 URL振り分け方針・§6 301リダイレクト確認事項（URL・リダイレクト要件）
  ...
  - §15 リリース時の確認事項（フェーズ・ロールバック・CS テンプレ棚卸し）
...
- 責務分離のため、別サブドメインへ切り出す場合の確認事項を整理する。
...
| SSO（別途判断） | Auth0等の共通認証基盤 | 最も堅牢 | 実装コスト大。別PRDで判断 |
...
- **静的アセット運用**。デプロイ構成、CDN 共有の有無、CI でのチェックサム比較要否は画面外の確認事項とする。
...
- PII（メール・電話番号等）の自動マスキング要否は画面外の確認事項とする。IPアドレス・フォーム送信内容を含むログは保管期間最短化、必要に応じ仮名化。
