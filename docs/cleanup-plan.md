...
## Current Status
- [2025-09-17] Archived legacy `src/` tree under `アーカイブ/legacy-src/` and relocated `tableManager.js.bak` out of `02_dashboard/src/`.
- [2025-09-17] Consolidated modal HTML by moving legacy `modals/` into `アーカイブ/legacy-modals/` alongside the active `02_dashboard/modals/` source.
- [2025-09-17] Migrated dashboard data fixtures into `data/` (core/responses/surveys/demos) and moved large dumps to `アーカイブ/data-dumps/`.
...
3. Large archival CSV/XLSX files move to `アーカイブ/data-dumps/` with a README describing origin and usage.
...
2. Move issue/PR body markdown to `docs/リファレンス/テンプレート/` and update any documentation references. **(Completed 2025-10-27)**
3. Gather temporary staging files (`temp/`, `temp_issue_body.txt`, `temp_review_comment.txt`) and either delete if obsolete or move to `アーカイブ/temp/`.
4. Relocate client-provided reference materials currently under `siryou/` and `06_other/` into `docs/リファレンス/リソース/` (or `docs/アーカイブ/`) with descriptive filenames.
