...
stateDiagram-v2
    [アンケート作成] --> [会期前]
    [会期前] --> [会期中 : 会期開始日時到達
    [会期中] --> 会期終了（データ化中） : 会期終了日時経過\nかつデータ化対象
    [会期中] --> 会期終了 : 会期終了日時経過\nかつデータ化対象外
    [会期終了（データ化中）] --> 会期終了（データ化完了） : データ化完了予定日10:00バッチ\nかつ完了フラグON
    [会期終了（データ化中）] --> 会期終了（データ化中） : データ化完了予定日10:00バッチ\nかつ完了フラグOFF
    [会期終了（データ化完了）] --> 会期終了（DL期限終了） : 会期終了後90日経過
    [会期終了] --> [会期終了（DL期限終了） : 会期終了後90日経過]
    [会期終了（DL期限終了）] --> [*]
'''