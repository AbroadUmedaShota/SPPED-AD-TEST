...
#### ①-2 今月の支払い確定額（全体サマリー）
全グループ総合の数値を表示。前月比 % 付き。

#### ①-3 絞り込み機能（フィルタ） & CSV出力
- **フィルタ:** 期間（Date Range）、展示会、指標フィルタ（閾値指定）。
- **CSVダウンロードボタン:**
...
| 項目 | 内容・計算式 | UI表現・備考 |
| :--- | :--- | :--- |
| **オペレーター名/所属** | 氏名 + グループ名 | アイコン表示 |
| **入力完了数** | 入力総数 | |
| **エラー件数** | 管理者修正分 | |
| **有効作業数** | `入力完了数 - エラー件数` | **支払い対象（最重要）** |
| **作業時間/平均** | `Total ◯h / Avg ◯s` | 併記する |
| **推定報酬額** | `有効作業数 × 設定単価` | 確定済みデータは固定 |
| **達成状況** | 指標設定に対するクリア状況 | **色分け/マーク表示**<br>・達成: <span style="color:blue">●</span> 青色強調<br>・未達: <span style="color:red">▲</span> 赤色強調 |
...
| :--- | :--- | :--- |
| **グループ名** | 会社名 | |
| **登録/稼働人数** | `稼働人数 / 登録人数` | 動員力 |
| **最大処理能力** | `平均処理数 × 稼働人数` | キャパシティ |
| **品質レベル** | 1枚平均速度 / 品質のばらつき | 当日分はばらつき非表示 |
| **コスパ** | CPY (Cards Per Yen) | 修正コスト(設定値)を加味して算出 |
| **納期遵守率** | `納期内完了数 ÷ 総数` | 指標として表示 |

### 4.3. 詳細モーダル仕様（Drill Down）

行（個人/グループ）をクリックするとモーダルで詳細を表示。
...
3. **レーダーチャート (Radar Chart):**
    ...
    - **データ:**
        - **【必須】** 項目ごとのエラー発生状況を分析するため、入力・照合フェーズにおいて**詳細なエラーログ（項目タイプごとの正誤判定結果）を確実に蓄積するデータ構造**とすること。
        - 単なる件数だけでなく、「どの項目でミスしたか」が追跡できるログテーブル（例: `RewardHistory` に `item_type` カラムを持たせる等）が必要。

#### 【グループ詳細モーダル】
1.  **グループサマリー & 比較:**
    - 最大能力、総コスト、平均品質に加え、**全体平均との比較**を表示。
2.  **所属オペレーター一覧テーブル:**
    - グループ内オペレーターの実績リスト。
...
3.  **散布図 (Scatter Plot):**
    ...
    - **分析:**
        - **左上:** 優秀層（Star - 速くて正確）
        - **右下:** パフォーマンス改善が必要な層（Critical Support Needed - 遅くて不正確）
        - 点の密集度で「品質のばらつき」を可視化。
